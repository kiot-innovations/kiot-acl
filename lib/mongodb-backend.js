  ...
  */
  union : function(bucket, keys, cb){
    ...
    try {
      let collection = this.db.collection(this.prefix + this.removeUnsupportedChar(collName));
      // Excluding bucket field from search result
      collection.find(searchParams ).project({_bucketname: 0}).toArray(function(err,docs){
        if(err instanceof Error) return cb(err);
  ...
  */
  add : function(transaction, bucket, key, values){
    ...
    transaction.push(function(cb){
      ...
      try {
        let collection = self.db.collection(self.prefix + self.removeUnsupportedChar(collName));
